...
async function part1() {
    ...
    const multipliedInstructions: number[] = instructions.map((instruction) => {
        const numbers: number[] = [...instruction[0].matchAll(/\d{1,3}/g)].map((number) =>
            parseInt(number[0])
        );
...
async function part2() {
    ...
    //get the instructions
    const instructions: RegExpExecArray[] = [
        ...instructionStrings.matchAll(/don't\(\)|do\(\)|mul\(\d{1,3},\d{1,3}\)/g),
    ];
    // console.log(instructions);
